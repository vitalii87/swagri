...
const MOBILE_UNKNOWN: u32 = u32::MAX;

static MOBILE_RUNNING: AtomicBool = AtomicBool::new(false);
static MOBILE_COMMANDS: OnceLock<Mutex<Option<mpsc::UnboundedSender<String>>>> = OnceLock::new();
static MOBILE_EVENTS: OnceLock<Mutex<VecDeque<String>>> = OnceLock::new();
static MOBILE_BATTERY_PERCENT: AtomicU32 = AtomicU32::new(MOBILE_UNKNOWN);
static MOBILE_CHARGING: AtomicU32 = AtomicU32::new(MOBILE_UNKNOWN);
static MOBILE_THERMAL_STATUS: AtomicU32 = AtomicU32::new(MOBILE_UNKNOWN);
static MOBILE_UNMETERED_NETWORK: AtomicU32 = AtomicU32::new(MOBILE_UNKNOWN);
...
struct Args {
    ...
    /// Explicit artifact quota used by embedded platforms without disk discovery.
    #[arg(long, hide = true)]
...
impl MobileAgentConfig {
    fn into_args(self) -> Result<Args> {
        let listen = "/ip4/0.0.0.0/udp/0/quic-v1"
            .parse()
            .context("invalid embedded listen address")?;
        Ok(Args {
            name: self.name,
            identity: Some(self.data_dir.join("identity.key")),
            listen,
            dial: Vec::new(),
            ...
            calibration: Some(self.data_dir.join("cpu-calibration.json")),
            artifact_store: Some(self.data_dir.join("artifacts")),
            artifact_storage_percent: 5.0,
            artifact_quota_bytes: Some(self.artifact_quota_bytes),
        })
    }
}
...
impl ResourceMonitor {
    ...
    fn refresh(&mut self) {
        ...
        self.snapshot.battery_percent = mobile_u8(&MOBILE_BATTERY_PERCENT);
        self.snapshot.charging = mobile_bool(&MOBILE_CHARGING);
        self.snapshot.thermal_status = mobile_u8(&MOBILE_THERMAL_STATUS);
        self.snapshot.unmetered_network = mobile_bool(&MOBILE_UNMETERED_NETWORK);
...
fn mobile_u8(value: &AtomicU32) -> Option<u8> {
    let value = value.load(Ordering::Relaxed);
    (value != MOBILE_UNKNOWN).then_some(value.min(u32::from(u8::MAX)) as u8)
}

fn mobile_bool(value: &AtomicU32) -> Option<bool> {
...
pub async fn run_cli() -> Result<()> {
    ...
    run_agent(args, command_rx, true).await
}

pub fn start_mobile_agent(config: MobileAgentConfig) -> Result<()> {
    if MOBILE_RUNNING
        .compare_exchange(false, true, Ordering::AcqRel, Ordering::Relaxed)
        .is_err()
    {
        bail!("mobile agent is already running");
    }
    let args = match config.into_args() {
        Ok(args) => args,
        Err(error) => {
            MOBILE_RUNNING.store(false, Ordering::Release);
            return Err(error);
        }
    };
    if let Err(error) = validate_resource_limits(&args) {
        MOBILE_RUNNING.store(false, Ordering::Release);
        return Err(error);
    }
    let (command_tx, command_rx) = mpsc::unbounded_channel();
    *MOBILE_COMMANDS
    ...
    let spawn_result = thread::Builder::new()
        .name("swagri-mobile-agent".into())
        .spawn(move || {
            init_tracing();
            let runtime = tokio::runtime::Builder::new_multi_thread()
                .enable_all()
                .thread_name("swagri-mobile-runtime")
                .build();
            let result = runtime
                .context("could not create mobile Tokio runtime")
                .and_then(|runtime| runtime.block_on(run_agent(args, command_rx, false)));
            if let Err(error) = result {
                emit_event("MOBILE_FAILED", &[&error.to_string()]);
            }
            *MOBILE_COMMANDS
                .get_or_init(|| Mutex::new(None))
                .lock()
    ...
    if let Err(error) = spawn_result {
        *MOBILE_COMMANDS
            ...
            .unwrap_or_else(|poisoned| poisoned.into_inner()) = None;
        MOBILE_RUNNING.store(false, Ordering::Release);
        return Err(error).context("could not start mobile agent thread");
    }
    Ok(())
}

pub fn send_mobile_command(command: impl Into<String>) -> Result<()> {
    let commands = MOBILE_COMMANDS
        .get_or_init(|| Mutex::new(None))
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    let sender = commands.as_ref().context("mobile agent is not running")?;
    sender
        .send(command.into())
        .context("mobile agent command channel is closed")
}

pub fn stop_mobile_agent() -> Result<()> {
    send_mobile_command("quit")
}

pub fn mobile_agent_running() -> bool {
    MOBILE_RUNNING.load(Ordering::Acquire)
}

pub fn update_mobile_environment(
    battery_percent: Option<u8>,
    charging: Option<bool>,
    thermal_status: Option<u8>,
    unmetered_network: Option<bool>,
) {
    MOBILE_BATTERY_PERCENT.store(
        battery_percent.map_or(MOBILE_UNKNOWN, u32::from),
        Ordering::Relaxed,
    );
    ...
    MOBILE_THERMAL_STATUS.store(
        thermal_status.map_or(MOBILE_UNKNOWN, u32::from),
        Ordering::Relaxed,
    );
    ...
    let safe_to_contribute =
        mobile_safe_to_contribute(battery_percent, charging, thermal_status, unmetered_network);
    let _ = send_mobile_command(if safe_to_contribute {
        "resume-resources"
    } else {
        "pause-resources"
    });
...
pub fn drain_mobile_events() -> Vec<String> {
    MOBILE_EVENTS
...
async fn run_agent(
    ...
    let artifact_quota = args.artifact_quota_bytes.map_or_else(
        || disk_quota_bytes(&artifact_path, args.artifact_storage_percent),
        Ok,
    )?;
    ...
    let source = if cfg!(target_os = "android") {
        ...
        Some(build_update_source(&keypair)?)
    };
    let debugger_source = if cfg!(target_os = "android") {
        None
    } else {
    ...
    loop {
        tokio::select! {
            maybe_line = command_rx.recv(), if !commands_closed => {
                match maybe_line {
                    Some(line) => {
                        if !handle_command(
                            &line,
                    ...
                    None if args.daemon => commands_closed = true,
                    None => break,
                }
            }
...
fn serve_update_request(request: &UpdateRequest, updates: &UpdateManager) -> UpdateResponse {
    match request {
        UpdateRequest::Manifest => updates.source.as_ref().map_or_else(
            || UpdateResponse::Error {
                message: "this platform does not share self-update binaries".into(),
            },
        ...
        UpdateRequest::Chunk {
            ...
            offset,
            length,
        } => updates.source.as_ref().map_or_else(
            || UpdateResponse::Error {
                message: "this platform does not share self-update binaries".into(),
            },
...
fn schedule_self_update(
    ...
    let Some(source) = updates.source.as_ref() else {
        emit_event(
            "UPDATE_FAILED",
            &[
                &peer.to_string(),
...
fn emit_event(kind: &str, fields: &[&str]) {
    let mut line = format!("SWAGRI_EVENT\t{kind}");
    for field in fields {
        line.push('\t');
        line.push_str(&field.replace(['\t', '\r', '\n'], " "));
    }
    println!("{line}");
    let mut events = MOBILE_EVENTS
        .get_or_init(|| Mutex::new(VecDeque::new()))
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    if events.len() >= 2_000 {
...
mod android_jni {
    use jni::{
        ...
        sys::{JNI_FALSE, JNI_TRUE, jboolean, jfloat, jint, jlong, jstring},
    };
    ...
    #[unsafe(no_mangle)]
    pub extern "system" fn Java_com_swagri_android_NativeBridge_nativeStart(
        mut env: JNIEnv<'_>,
        _object: JObject<'_>,
        data_dir: JString<'_>,
        name: JString<'_>,
    ...
    #[unsafe(no_mangle)]
    pub extern "system" fn Java_com_swagri_android_NativeBridge_nativeSend(
        mut env: JNIEnv<'_>,
        _object: JObject<'_>,
        command: JString<'_>,
    ) -> jboolean {
        java_string(&mut env, command)
            .and_then(send_mobile_command)
            .map_or(JNI_FALSE, |()| JNI_TRUE)
    }

    #[unsafe(no_mangle)]
    pub extern "system" fn Java_com_swagri_android_NativeBridge_nativeStop(
        _env: JNIEnv<'_>,
        _object: JObject<'_>,
    ) -> jboolean {
        stop_mobile_agent().map_or(JNI_FALSE, |()| JNI_TRUE)
    }

    #[unsafe(no_mangle)]
    pub extern "system" fn Java_com_swagri_android_NativeBridge_nativeIsRunning(
        _env: JNIEnv<'_>,
        _object: JObject<'_>,
    ) -> jboolean {
        if mobile_agent_running() {
            JNI_TRUE
        } else {
            JNI_FALSE
        }
    ...
    #[unsafe(no_mangle)]
    pub extern "system" fn Java_com_swagri_android_NativeBridge_nativePoll(
        env: JNIEnv<'_>,
        _object: JObject<'_>,
    ) -> jstring {
        let joined = drain_mobile_events().join("\n");
    ...
    #[unsafe(no_mangle)]
    pub extern "system" fn Java_com_swagri_android_NativeBridge_nativeUpdateEnvironment(
        _env: JNIEnv<'_>,
        _object: JObject<'_>,
        battery_percent: jint,
        charging: jboolean,
        thermal_status: jint,
        unmetered_network: jboolean,
    ) {
        update_mobile_environment(
            u8::try_from(battery_percent).ok(),
            Some(charging == JNI_TRUE),
            u8::try_from(thermal_status).ok(),
            Some(unmetered_network == JNI_TRUE),
        );
    }
}
